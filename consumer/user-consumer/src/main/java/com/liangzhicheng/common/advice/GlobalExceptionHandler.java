...
public class GlobalExceptionHandler extends BaseController {
    ...
    public ResponseResult exception(Exception e) {
        ...
        if(e instanceof MissingServletRequestParameterException){
            ...
            return super.buildFailedInfo("参数缺失异常，请检查");

        }else if(e instanceof TypeMismatchException){

            PrintUtil.error("参数类型异常，异常信息为：{}", errorMessage);
            return super.buildFailedInfo("参数类型异常，请检查");

        }else if(e instanceof InvalidFormatException){

            PrintUtil.error("参数格式异常，异常信息为：{}", errorMessage);
            return super.buildFailedInfo("参数格式异常，请检查");

        }else if(e instanceof BadSqlGrammarException){

            PrintUtil.error("未知的字段异常，异常信息为：{}", errorMessage);
            return super.buildFailedInfo("未知的字段异常，请检查");

        }else if(e instanceof JsonParseException){

            PrintUtil.error("JSON格式异常，异常信息为：{}", errorMessage);
            return super.buildFailedInfo("JSON格式异常，请检查");

        }else if(e instanceof DataIntegrityViolationException){

            PrintUtil.error("保存异常，异常信息为：{}", errorMessage);
            return super.buildFailedInfo("保存异常，请检查");

        }else if(e instanceof TransactionException){

            PrintUtil.error("事务处理异常，异常信息为：{}", errorMessage);
            return super.buildFailedInfo("事务处理异常，请检查");
        }
